...
def complete_feature_selection(X_train_num,X_test_num,y_train):
    try:
        # normalize column names (remove stray spaces) to avoid name mismatches
        X_train_num = X_train_num.copy()
        X_test_num = X_test_num.copy()
        X_train_num.columns = X_train_num.columns.str.strip()
        X_test_num.columns = X_test_num.columns.str.strip()
        ...
        logger.info(
            f'Columns we need to remove from constant technique : {X_train_num.columns[reg_con.get_support()]}')
        good_data = reg_con.transform(X_train_num)
        good_data_1 = reg_con.transform(X_test_num)
        # use TRAIN columns (those seen at fit time) when creating test dataframe
        X_train_num_fs = pd.DataFrame(data=good_data, columns=X_train_num.columns[reg_con.get_support()])
        X_test_num_fs = pd.DataFrame(data=good_data_1, columns=X_train_num.columns[reg_con.get_support()])
        #quasi constant
        reg_quasi.fit(X_train_num_fs)
        logger.info(
            f'Columns we need to remove from quansi constant technique : {X_train_num_fs.columns[reg_quasi.get_support()]}')
        ...
        # again use TRAIN (after-quasi) columns for test dataframe
        X_test_num_fs_2 = pd.DataFrame(data=good_data_3, columns=X_train_num_fs.columns[reg_quasi.get_support()])
        ...
        logger.info(f' after : {X_train_num_fs_1.columns} --> {X_train_num_fs_1.shape}')
        logger.info(f'after : {X_test_num_fs_2.columns} --> {X_test_num_fs_2.shape}')
        ...
        logger.info(f'Error in line no{error_line.tb_lineno} : due to error {error_msg}')
