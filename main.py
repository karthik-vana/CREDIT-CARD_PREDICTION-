...
class CREDIT_CARD_DATA:
    def __init__(self, path):
        try:
            ...
            logger.info(f'Total no of columns : {self.df.shape[1]}')
    ...
    def cat_to_num(self):
        try:
            ...
            logger.info(f"{self.X_training_data.shape}")
            logger.info(f"{self.X_testing_data.shape}")

            logger.info(f"{self.X_training_data.isnull().sum()}")
            logger.info(f"{self.X_testing_data.isnull().sum()}")

            logger.info('===========================================')

            logger.info(f"{self.X_training_data.sample(10)}")
            logger.info(f"{self.X_testing_data.sample(10)}")
    ...
    def data_b(self):
        try:
            ...
            balance_data(self.X_training_data,self.y_train,self.X_testing_data,self.y_test)
        except Exception as e:
            error_type, error_msg, error_line = sys.exc_info()
            logger.info(f'Error in Line no : {error_line.tb_lineno}: due to {error_msg}')
...
if __name__ == "__main__":
    try:
        obj = CREDIT_CARD_DATA('C:\\Users\\Karthik\\Downloads\\ML_Main-Project\\Ml_pipeline\\creditcard.csv')
        obj.missing_values()
        obj.vt_hol()
        obj.fs()
        obj.cat_to_num()
        ...
        error_type, error_msg, error_line = sys.exc_info()
        print(f'Error in line no{error_line.tb_lineno} : due to error {error_msg}')